...
const EditStudent = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await fetch(`/get/student/profile/${id}`)
    ...
    useEffect(() => {
        if (fetchdata) {
            setFormData({
                ...
                reg_no: fetchdata.reg_no||'',
                session: fetchdata.session||'',
                linkedIn: fetchdata.linkedIn||'',
                skills: fetchdata.skills||[],
                photo: fetchdata.photo||null
            })
            setPreviewUrl(fetchdata.photo)
        }
    },[fetchdata])
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`/api/student/update/${id}`, {
                method: 'POST',
                body: data
            });
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-slate-100">
                    <header className="mb-10">
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Edit Profile <span className="text-blue-600">Student</span></h1>
